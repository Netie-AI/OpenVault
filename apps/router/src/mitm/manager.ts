...
export function interpretMitmStartupError(stderr: string, port: number): string {
  ...
  if (lower.includes("router_api_key")) {
    return "MITM server failed to start: no API key was provided (ROUTER_API_KEY is required). Set a router API key in FreeRoute and retry.";
  }
...
/**
 * Start MITM proxy
 * @param {string} apiKey - FreeRoute API key
...
async function startMitmInternal(
  ...
  // is shared with the FreeRoute process: getIngestTokenForBootstrap() returns
