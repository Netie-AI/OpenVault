...
/**
 * Identifies only entries owned by FreeRoute. Generic custom endpoints are not
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (FreeRoute)`,
...
/**
 * Merge one FreeRoute model into Qwen Code's current V4 settings contract.
...
/** Remove only FreeRoute-owned Qwen Code entries and selection state. */
