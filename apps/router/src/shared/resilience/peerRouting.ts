...
export const OMNIROUTE_PEER_TRACE_HEADER = "X-FreeRoute-Peer-Trace";
...
export function inspectPeerRequest(
  ...
  if (trace.includes(instanceId)) {
    return {
      ...
      message: "FreeRoute peer routing loop detected",
    };
  }
  if (trace.length >= getMaxPeerHops(env)) {
    return {
      code: "peer_hop_limit_exceeded",
      message: "FreeRoute peer routing hop limit exceeded",
    };
  }
  return null;
}
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted FreeRoute URL.
