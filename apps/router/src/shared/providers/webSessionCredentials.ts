...
export const WEB_SESSION_CREDENTIAL_REQUIREMENTS = {
  ...
  "zai-web": {
    ...
    hintFallback:
      'Copy only the "token" value from chat.z.ai Local Storage. Do not copy a Cookie header. FreeRoute uses its browser transport to obtain the per-request CAPTCHA proof.',
    guideSteps: [
      ...
      "Paste the token below and check the connection. FreeRoute handles the per-request CAPTCHA through its browser transport.",
    ],
    guideNote:
  ...
  maxai: {
    ...
    placeholder:
      "Use browser sign-in — FreeRoute mints the MaxAI access token, device id, and user id for you",
    acceptsFullCookieHeader: false,
    storageKeys: [
      "accessToken",
      "access_token",
