...
// ── Check if a base_url points to FreeRoute ──────────────────────────────
...
// ── Check if FreeRoute is configured ─────────────────────────────────────
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-FreeRoute endpoint
...
// ── POST - Apply FreeRoute as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-FreeRoute provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-FreeRoute conflicts, back up before overwrite ──
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a FreeRoute model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(error) } }, { status: 500 });
...
// ── DELETE - Remove FreeRoute configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "FreeRoute config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "FreeRoute config removed. Restart Letta CLI to take effect.";
