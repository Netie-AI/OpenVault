...
// POST — write FreeRoute settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge FreeRoute settings (pi uses OpenAI-compatible config)
...
// DELETE — remove FreeRoute settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi FreeRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
