...
// Check if config has FreeRoute settings
...
// POST - Update FreeRoute settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only FreeRoute related fields (api_key goes to auth.json, not config.toml)
...
// DELETE - Remove FreeRoute settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove FreeRoute related root fields
    delete parsed._root.openai_base_url;
