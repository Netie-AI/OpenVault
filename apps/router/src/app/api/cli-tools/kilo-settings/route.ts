...
// Check if FreeRoute OpenAI-compatible provider is configured
...
// POST - Configure Kilo Code to use FreeRoute as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update FreeRoute as openai-compatible provider
...
// DELETE - Remove FreeRoute config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "FreeRoute settings removed from Kilo Code",
    });
  } catch (error) {
