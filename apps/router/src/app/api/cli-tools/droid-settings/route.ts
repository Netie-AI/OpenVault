...
// Check if settings has FreeRoute customModels.
// Multi-model entries are stored as `custom:FreeRoute-0`, `custom:FreeRoute-1`, …
...
// POST - Update FreeRoute customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing FreeRoute config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isOmniRouteCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend FreeRoute entries (one per requested model)
...
// DELETE - Remove FreeRoute customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove FreeRoute customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isOmniRouteCustomModel(m));
    ...
    return NextResponse.json({
      success: true,
      message: "FreeRoute settings removed successfully",
    });
  } catch (error) {
