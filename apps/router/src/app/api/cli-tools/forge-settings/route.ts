...
/**
 * Render the FreeRoute provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by FreeRoute (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-FreeRoute marker comment.
 */
...
// POST — write FreeRoute settings to Forge config.toml
...
// DELETE — remove Forge FreeRoute config
