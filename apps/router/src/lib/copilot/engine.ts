...
FreeRoute supports 212+ providers across categories:
...
I can use CodeGraph to explore the FreeRoute codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## FreeRoute Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`freeroute health\`
- "CLI list-combos" → ejecuta \`freeroute list-combos\`
- "CLI set-budget 10" → ejecuta \`freeroute set-budget 10\`

### Conocimiento
- "Cómo funciona FreeRoute?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with FreeRoute.\n\n${getHelpResponse()}`,
    };
  }
