...
/**
 * Build a clean child env for Claude Code pointed at FreeRoute.
 *
...
export function buildClaudeEnv(baseEnv, baseUrlOrPort, authToken, opts = {}) {
  ...
  // from stopping at its local login gate before it ever contacts FreeRoute (an
...
/**
 * Resolve the FreeRoute base URL + auth for launch, honouring (in order):
...
export function registerLaunch(program) {
  program
    .command("launch")
    .description(
      t("launch.description") || "Launch Claude Code pointed at FreeRoute (local or remote)"
    )
    .option("--port <port>", t("serve.port") || "Proxy port", "20128")
    .option("--remote <url>", "Remote FreeRoute base URL (overrides --port and the active context)")
    .option(
      "--profile <name>",
    ...
    .option("--api-key <key>", "Alias for --token (FreeRoute access token / API key)")
