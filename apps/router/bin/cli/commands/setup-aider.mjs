/**
 * freeroute setup-aider — configure Aider (aider.chat) for FreeRoute.
 *
...
export async function runSetupAiderCommand(opts = {}) {
  ...
  const guard = await guardHostConfigTarget(configPath, {
    toolLabel: "Aider",
    hostCommand: "freeroute setup-aider",
    allowContainerWrite: Boolean(opts.allowContainerWrite ?? opts["allow-container-write"]),
    dryRun,
  });
  if (guard !== 0) return guard;

  printHeading("FreeRoute → Aider (openai-compatible via LiteLLM)");
...
export function registerSetupAider(program) {
  program
    .command("setup-aider")
    .description("Configure Aider for FreeRoute: write ~/.aider.conf.yml + print the env recipe")
