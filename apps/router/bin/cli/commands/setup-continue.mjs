/**
 * freeroute setup-continue — configure Continue (continue.dev) for FreeRoute.
 *
...
/**
 * Merge FreeRoute models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "FreeRoute Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("FreeRoute → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} FreeRoute model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} FreeRoute models)`);
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the FreeRoute model catalog"
    )
