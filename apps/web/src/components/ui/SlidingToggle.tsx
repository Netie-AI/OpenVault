...
export function SlidingToggle<T extends string = string>({
  ...
  // The pill is absolutely positioned, so it has to be measured off the real
  // buttons after layout — label widths differ and are not known up front.
  useEffect(() => {
      ...
      {options.map((option, index) => {
        ...
        return (
          ...
          >
            ...
            >
              {hasIcon && <span className={config.iconSize}>{option.icon}</span>}
