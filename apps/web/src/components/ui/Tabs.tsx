...
export interface TabDef<K extends string = string> {
  ...
  /** When true the tab is not rendered (e.g. a tab that only applies to some devices). */
  hidden?: boolean;
}
...
/**
 * Underline tab strip. Controlled: the caller owns the active `value`.
 */
export function Tabs<K extends string>({ tabs, value, onChange, className = "" }: TabsProps<K>) {
  return (
    <div className={`flex items-center gap-1 overflow-x-auto border-b border-border/50 ${className}`}>
      {tabs
