...
export function CustomSelect<T extends string>({
  ...
  // The menu is portalled to <body>, so it cannot inherit the trigger's
  // position — it is measured against the viewport on open, on resize, and on
  // any scroll (capture phase, because the scroller may be an ancestor).
    ...
    <div ref={containerRef} className={`relative ${className}`}>
      <button
        ...
        className={`
          ...
          border border-border/50 disabled:cursor-not-allowed disabled:opacity-50
          ${isOpen ? "bg-muted/80 border-border" : "bg-muted/40 hover:bg-muted/60 hover:border-border"}
        `}
        type="button"
      >
