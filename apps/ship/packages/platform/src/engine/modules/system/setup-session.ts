...
export function promptSetupUser(sessionId: string, prompt: PromptPayload): Promise<string> {
  ...
  const waiting = promptRegistry.wait(sessionId, () => { session.pendingPrompt = undefined; });
...
/** The owned engine closes its timers and pending decisions after work drains. */
export function closeSetupSessions(): void {
  clearInterval(heartbeatTimer);
  promptRegistry.dispose();
  for (const session of sessions.values()) removeSetupSession(session.id);
  sessions.dispose();
}
