...
import { resolveDeploymentRuntimeForRead, disposeRuntime } from "@repo/platform/engine/lib/deployment-runtime";
import { livePrimaryContainerId } from "@repo/platform/engine/modules/services/service-container";
import {
...
export async function getDashboardStats(ctx: RequestContext) {
  ...
  if (ctx.tokenScope || ctx.role === "restricted") {
    projectIds = [];
    // Count only visible projects, before aggregation; a project grant is not
    // permission to discover the rest of an organization's deployment activity.
    for (let page = 1; ; page++) {
      const result = await repos.project.listByOrganization(ctx.organizationId, { page, perPage: 500 });
      for (const project of result.rows) {
        if (await authorization.checkPermissionOnResource({ ...ctx, scopeMode: "fixed" }, { resourceType: "project", resourceId: project.id, action: "read" })) projectIds.push(project.id);
      }
      if (page * 500 >= result.total || result.rows.length === 0) break;
    }
  }
  const [projectCounts, deploymentsByStatus] = await Promise.all([
    repos.project.countByOrganization(ctx.organizationId, projectIds),
    repos.deployment.countByStatusForOrganization(ctx.organizationId, projectIds),
  ]);
