...
import { checkPermissionOnResource } from "@repo/platform/engine/lib/authorization";
import {
...
export async function listOrganizationIssues(
  ...
  if (status === "resolved") {
    ...
    const issues = [];
    for (const row of rows) if (await visibleIncident(row)) issues.push(incidentIssue(row, names));
  ...
  for (const issue of components?.servers ?? []) {
    if (!(await visible("server", issue.server.id))) continue;
  ...
  for (const row of behind) {
    if (!(await visible("server", row.serverId))) continue;
  ...
  for (const [projectId, actions] of pending) {
    if (!(await visible("project", projectId))) continue;
