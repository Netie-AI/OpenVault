...
export async function getLocalGhToken(): Promise<string | null> {
  ...
  // Stored credentials were explicitly supplied to this installation. Reading
  // the embedding host's gh configuration needs a separate host policy grant.
  if (!nativeHostExecutionEnabled()) return null;
...
async function runDeviceFlow(
  ...
  while (activeFlows.has(flowKey)) await cancelDeviceFlow(flowKey);
  ...
  return new Promise<Verification>((resolveVerification, rejectVerification) => {
    let deadline = setTimeout(() => state.abort.abort(new Error("GitHub device authorization timed out")), 30_000);
    deadline.unref?.();
    const auth = createOAuthDeviceAuth({
      clientId,
      clientType: "oauth-app",
      scopes: ["repo", "read:org", "read:user"],
      request: request.defaults({ request: { signal: state.abort.signal } }),
      onVerification: (verification) => {
        state.abort.signal.throwIfAborted();
        clearTimeout(deadline);
        deadline = setTimeout(() => state.abort.abort(new Error("GitHub device authorization expired")), Math.min(verification.expires_in, 900) * 1000);
        deadline.unref?.();
    ...
    state.task = trackBackgroundWork(auth({ type: "oauth" })
      .then(async (result) => {
        if (state.abort.signal.aborted || activeFlows.get(flowKey) !== state) return;
 ...
 */
export async function startDeviceFlow(ctx: ExecutionContext): Promise<Verification> {
  if (env.CLOUD_MODE) throw new Error("Device flow is not available in cloud mode");
  ...
  return runDeviceFlow(ctx.userId, async token => {
    const current = await resolveExecutionAuthority(authority, "github-device-flow");
    await instanceAuthorization.assert(current, "write");
...
export function getDeviceFlowStatus(userId: string): {
  ...
  if (state.status === "complete") {
    // The completed credential lives only in its encrypted destination.
 ...
 */
export async function cancelDeviceFlow(userId: string): Promise<void> {
  const state = activeFlows.get(userId);
  if (!state) return;
  state.abort.abort(new Error("GitHub device authorization was canceled"));
  await state.task;
  if (activeFlows.get(userId) === state) activeFlows.delete(userId);
}

export async function cancelInstanceDeviceFlows(): Promise<void> {
  await Promise.all([...activeFlows.keys()].filter(key => !key.startsWith("server:")).map(cancelDeviceFlow));
}

export async function closeDeviceFlows(): Promise<void> {
  while (activeFlows.size) await Promise.all([...activeFlows.keys()].map(cancelDeviceFlow));
}
