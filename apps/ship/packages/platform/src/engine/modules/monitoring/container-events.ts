...
export async function renewEventWatchers(groupKeys: readonly string[]): Promise<void> {
  ...
  for (const key of groupKeys) {
    ...
    armLease(sub);
    SUBS.set(key, sub);
    void trackBackgroundWork(connect(sub));
  }
}
...
function armLease(sub: Subscription): void {
  ...
  sub.expiry = unref(
    setTimeout(() => {
      sub.expiry = null;
      void trackBackgroundWork(teardown(sub, "lease expired"));
    }, LEASE_MS),
  );
}

/** Shutdown: drop every stream and hand its SSH connection back to the pool. */
export async function stopAllContainerEventWatchers(opts: { closing?: boolean } = {}): Promise<void> {
  if (opts.closing) closing = true;
...
async function connect(sub: Subscription): Promise<void> {
  ...
  try {
    ...
    sub.stopStream = await runtime.watchContainerEvents({
      ...
      onClose: (err) => void trackBackgroundWork(onClose(sub, err)),
    });
...
function schedule(sub: Subscription, opts?: { delayMs: number; enforceFloor: boolean }): void {
  ...
  sub.debounce = unref(
    setTimeout(() => {
      sub.debounce = null;
      void trackBackgroundWork(sweep(sub));
    }, wait),
  );
}
