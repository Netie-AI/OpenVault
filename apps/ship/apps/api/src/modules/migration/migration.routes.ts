...
r.post("/adopt", { tag: "server:write", collection: true, mcp: { description: "Register selected discovered Docker services as a FreeBuild project while preserving running containers and volumes. Rediscovers values on the server; do not send masked secrets as replacements. Deployment and cutover are separate." }, body: MigrationRequestSchemas.adopt }, migration.adoptServer);
// Re-import an orphaned Openship project (DR / cross-instance), preserving its id.
r.post("/reimport", { tag: "server:write", collection: true, mcp: { description: "Re-register an orphaned FreeBuild project found on this server, preserving its scanned project ID and configuration. Use scan to identify the project; this is recovery of existing workloads." }, body: MigrationRequestSchemas.reimport }, migration.reimportServer);
