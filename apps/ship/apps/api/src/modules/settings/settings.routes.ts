...
r.patch("/route-strategy", { tag: "settings:write", auditHandledByOperation: true, body: UpdateRouteStrategyBody, mcp: { description: "Set the default edge→app route strategy (auto / loopback-port / container-ip)." } }, ctrl.updateRouteStrategy);
...
r.patch("/transfer", { tag: "settings:write", auditHandledByOperation: true, body: UpdateTransferPrefsBody, mcp: { description: "Set the default volume-transfer mode (auto/stream/direct/rsync) and compression (auto/zstd/gzip/none) for migrations." } }, ctrl.updateTransferPrefs);

/** PATCH /forward-git - flip the generic forward-git-identity-to-remote-server preference */
r.patch("/forward-git", { tag: "settings:write", auditHandledByOperation: true, body: UpdateForwardGitBody, mcp: { description: "Enable/disable forwarding your local git identity (gh CLI) to remote build servers during a server clone." } }, ctrl.updateForwardGitToServer);
