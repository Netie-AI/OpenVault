...
describe("serviceLogSource", () => {
  it("keeps historical deployment output even if that service still exists", () => {
    expect(serviceLogSource({ deploymentStatus: "ready", serviceStatus: "running", isCurrentDeployment: false })).toBe("build");
  });
  it("routes a running service to the runtime stream once the deploy is ready", () => {
    expect(serviceLogSource({ isCurrentDeployment: true, deploymentStatus: "ready", serviceStatus: "running" })).toBe(
      "runtime",
    );
  });
  ...
  it("keeps build logs while the deployment is still in progress", () => {
    for (const deploymentStatus of ["building", "deploying"] as const) {
      expect(serviceLogSource({ isCurrentDeployment: true, deploymentStatus, serviceStatus: "running" })).toBe("build");
    }
  });

  it("never routes to runtime under a held keep/reject decision", () => {
    expect(
      serviceLogSource({ isCurrentDeployment: true,
  ...
  it("keeps build logs for services that are not running after success", () => {
    for (const serviceStatus of ["building", "deploying", "built", "failed", undefined] as const) {
      expect(serviceLogSource({ isCurrentDeployment: true, deploymentStatus: "ready", serviceStatus })).toBe("build");
    }
  });
