...
describe("log processor end frames", () => {
  it("reports a transport failure without treating it as a clean exit", () => {
    const onError = vi.fn();
    const onContainerExit = vi.fn();
    const processor = createLogMessageProcessor({ onError, onContainerExit });
    const frame = processor.parseMessage({ type: "end", error: "SSH disconnected" });
    processor.handleMessage(frame!, { rawText: "" } as any);
    expect(onError).toHaveBeenCalledWith("SSH disconnected");
