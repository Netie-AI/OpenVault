...
| Skills library / distill ingest / Cortex crew | OpenVault half shipped (`/api/crew/gate`, `netie-kb.skills` signpost at `:8030`). Skill **bodies** belong in Netie-KB (R-0016), not Cortex packs and not this vault. Unlock: grant `Netie-AI/Cortex`, `netie-control`, `Netie-KB` so the registry can be the generator (R-0009) and Cortex-crew can be merged if gates are green. |
