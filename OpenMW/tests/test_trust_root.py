...
def test_service_registration_allows_env_cidr(home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ok = allowed.post("/keys/services", json={"service_id": "dms"}, headers={**INTENT, **headers})
    assert ok.status_code == 200, ok.text
    blocked = denied.post(
        "/keys/services", json={"service_id": "other"}, headers={**INTENT, **headers}
    )
...
def test_invalid_services_allow_entry_is_skipped(
    ...
    still_denied = _client(host="203.0.113.9").post(
        "/keys/services", json={"service_id": "ghost"}, headers={**INTENT, **headers}
    )
