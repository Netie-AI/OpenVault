...
# DR-0008 - Claude/Cursor work split protocol

## Context and Problem Statement

Claude and Cursor were both editing the same repository without a stable division of
labor, risking duplicated rework and architecture decisions being made by whichever agent
reached a file first.

## Considered Options

- No formal split — coordinate ad hoc in chat each session
- A static, one-time task list with no ongoing protocol
- An explicit, priority-ordered owner-per-task split plus a shared lock file for hot paths
...
- Good: prevented both agents from redesigning the same surface in parallel; STATUS.md's
  later entries show queued items landing without rework.
- Bad: the "hot files" claim table went stale within days (last updated 2026-07-24) and
  this cleanup found it still showing two lanes as "active" a week and a half later, with
  no one having released them.
